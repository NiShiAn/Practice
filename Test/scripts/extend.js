//document.write("<script type='text/javascript' src='/scripts/plus/pinyin/dict/pinyin_dict_firstletter.js'></script>");
//document.write("<script type='text/javascript' src='/scripts/plus/pinyin/dict/pinyin_dict_notone.js'></script>");
//document.write("<script type='text/javascript' src='/scripts/plus/pinyin/dict/pinyin_dict_polyphone.js'></script>");
//document.write("<script type='text/javascript' src='/scripts/plus/pinyin/dict/pinyin_dict_withtone.js'></script>");  
...
$.extend({
    ...
    zhPinyin: function (target) {
        ...
        var hanzis = Object.keys(hanziToPinyin);
        var pinyins = Object.values(hanziToPinyin);
        // 缓存
        var cache = Object.assign({},hanziToPinyin,fixedWrongPinyinHanzis);
        // safari 浏览器修正一些比较汉字
    ...
    chPinyin: function (str) {
            ...
            getPinyin: function (chinese, splitter, withtone, polyphone) {
                ...
                if (dict.withtone) // 优先使用带声调的字典文件
                {
                    for (var i = 0, len = chinese.length; i < len; i++) {
                        if (typeof def[chinese[i]] != "undefined") {
                            result.push(def[chinese[i]]);
                            continue;
                        }
                ...
                else if (dict.notone) // 使用没有声调的字典文件
                {
                    ...
                    for (var i = 0, len = chinese.length; i < len; i++) {
                        if (typeof def[chinese.charAt(i)] != "undefined") {
                            result.push(def[chinese[i]]);
                            continue;
                        }
        ...
        return pinyinUtil.getPinyin(str, ",", false);
    }
});