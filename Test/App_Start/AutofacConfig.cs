...
namespace Test
{
    public class AutofacConfig
    {
        ...
        public static void Register()
        {
            //初始化AutoFac的相关功能  
            /* 
             1.0 告诉AutoFac初始化数据仓储层Test.dll中所有类的对象实例。这些对象实例以其接口的形式保存在AutoFac容器中 
             2.0 告诉AutoFac初始化业务逻辑层Test.COM.dll中所有类的对象实例。这些对象实例以其接口的形式保存在AutoFac容器中 
            ...
            //2.1 从当前运行的bin目录下加载Test.dll程序集  
            var controllerAss = Assembly.Load("Test");

            //2.2 告诉AutoFac控制器工厂，控制器的创建从controllerAss中查找
            //（注意：RegisterControllers()方法是一个可变参数，如果你的控制器类的创建需要去多个程序集中查找的话，那么我们就再用Assembly controllerBss=Assembly.Load("需要的程序集名")加载需要的程序集，然后与controllerAss组成数组，然后将这个数组传递到RegisterControllers()方法中）  
            ...
            //3.1 加载数据仓储层Test.COM这个程序集。  
            var repositoryAss = Assembly.Load("Test.COM");
            //3.2 反射扫描这个Test.COM.dll程序集中所有的类，得到这个程序集中所有类的集合。  
            var rtypes = repositoryAss.GetTypes();
            ...
            //3.4 加载业务逻辑层Test.DAO这个程序集。  
            //var servicesAss = Assembly.Load("Test.DAO");
            //3.5 反射扫描这个Test.DAO.dll程序集中所有的类，得到这个程序集中所有类的集合。  
            //var stypes = servicesAss.GetTypes();
